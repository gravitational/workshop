...
All metrics collected by Heapster are placed into the `k8s` database in InfluxDB. See below for a list of metrics collected by Heapster. Each metric is stored as a separate “series” in InfluxDB. Heapster also tags each metrics with different labels, such as host name, pod name, container name and others, which become “tags” on the stored series. Tags are indexed so queries on tags are fast.
...
During initial installation Gravity pre-configures InfluxDB with the following retention policies:
...
In addition, some of the most commonly helpful metrics (such as CPU/memory usage, network transfer rates) are rolled up to lower resolutions and stored using the longer retention policies mentioned above.
