...
This will produce a JSON document with information about all individual probes run by Satellite and overall cluster status.

### System Logs

All modern Linux distributions use systemd as their init system. A node with Gravity installed on it runs two specific systemd units: Planet and Teleport. The Planet service is a container itself and runs all Kubernetes components inside it. The Teleport service provides controlled SSH access to the node.
...
```bash
node-1# sudo systemctl list-units | grep planet | grep service
gravity__gravitational.io__planet__5.6.0-11305-10-g9e464aa.service
...
Apart from systemd logs, it is useful to look at other log files in the host’s /var/log directory. As a particular example, on systems where SELinux is enabled /var/log/audit/audit.log contains information about enforced policies which can sometimes block certain executables (e.g. docker) or paths breaking the cluster in mysterious ways.
...
Overlay network is an essential component of a Kubernetes cluster that allows pods to communicate with each other. By default the overlay network is provided by Flannel with VXLAN backend. Flannel uses in-kernel VXLAN encapsulation (using UDP as a transport protocol) which in the default configuration uses port 8472. (Note that since this is a kernel feature, the port is not visible in the netstat output.)

The usual symptom of the overlay network malfunction is when pods on one node can’t talk to pods on other nodes while pods colocated on the same node can still talk just fine. Let’s demonstrate this by blocking VXLAN port:
...
Overlay network may be broken for a variety of other reasons too, one being flannel not able to register routes for subnets which is a more common occurrence in the cloud environments, for example if the node does not have sufficient cloud API permissions. In such cases, flannel will log an issue in its log:

```bash
node-1-planet# journalctl -u flanneld --no-pager
```

Let’s repair overlay network in our cluster in the meanwhile and make sure it recovers:
...
Another set of pretty common issues a cluster can encounter are related to DNS resolution. Gravity uses CoreDNS as an in-cluster DNS service. DNS issues usually manifest themselves in the pods not being able to talk to other pods via service names due to resolution failures, while the overlay network itself keeps functioning.
...
In more recent Gravity versions, the clock difference check is also built into Satellite and if it’s too high, `gravity status` will show it.
